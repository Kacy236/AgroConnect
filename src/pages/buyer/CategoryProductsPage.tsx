...
export function CategoryProductsPage() {
  ...
  const products = useMemo(
    () =>
      query.trim()
        ? search(query, slug as CategorySlug)
        : byCategory(slug as CategorySlug),
      ...
      <div className="pb-10">
        <div className="px-4 lg:px-8">
          <SearchInput
            ...
            wrapClassName="lg:max-w-lg"
          />

          <nav
            aria-label="Categories"
            className="no-scrollbar -mx-4 mt-4 flex gap-2.5 overflow-x-auto px-4 lg:mx-0 lg:px-0"
          >
            {CATEGORIES.map((c) => (
              <Link
                key={c.slug}
        ...
        {products.length ? (
          <div className="mt-4 lg:grid lg:grid-cols-2 lg:gap-3 lg:px-8 xl:grid-cols-3">
