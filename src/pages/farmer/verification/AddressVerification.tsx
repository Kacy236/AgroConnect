      ...
      <form id="address-form" onSubmit={submit} className="space-y-5">
        ...
        <section className="rounded-xl border border-brand-100 bg-brand-50/70 p-4">
          ...
          <ul className="mt-2 space-y-1.5">
            {REQUIREMENTS.map((r) => (
              <li key={r} className="flex items-center gap-2 text-[15px] text-ink">
            ...
            {DOC_TYPES.map(({ label, icon: Icon, tint }) => {
              ...
              return (
                <button
                  ...
                  className={cx(
                    'flex flex-col items-center gap-2 rounded-xl border px-2 py-4 text-center transition',
                    active
        ...
        <div>
          ...
          <UploadBox
            className="mt-1.5"
            tone="green"
            icon={<UploadCloud className="h-9 w-9 text-ink" strokeWidth={1.5} />}
