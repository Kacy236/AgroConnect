...
function Tick({ done }: { done: boolean }) {
  return (
    <svg viewBox="0 0 20 20" className={cx('h-5 w-5', done ? 'text-brand-600' : 'text-ink-line')}>
      <path
        fill="currentColor"
      ...
      <div className="space-y-6">
        <ul className="space-y-3 rounded-xl bg-brand-50/60 p-4">
          {checks.map((c) => (
            <li
              key={c.label}
              className="flex items-center justify-between gap-3 rounded-lg border border-ink-line bg-white px-4 py-3"
            >
              ...
              <span className="flex items-center gap-2">
                <span
                  className={cx(
                    ...
                    c.done ? 'text-brand-600' : 'text-ink-mute',
                  )}
                >
