...
const RULES = [
  ...
  { label: 'Include uppercase & lowercase', test: (v: string) => /[a-z]/.test(v) && /[A-Z]/.test(v) },
  { label: 'Include numbers', test: (v: string) => /\d/.test(v) },
]
              ...
              return (
                <li
                  ...
                  className={cx(
                    ...
                    met ? 'text-ink' : 'text-ink-soft',
                  )}
                >
                  <Check
                    className={cx('h-4 w-4 shrink-0', met ? 'text-brand-600' : 'text-ink-mute')}
                    strokeWidth={3}
                  />
                  {rule.label}
                </li>
            ...
            <div className="grid grid-cols-2 gap-4">
              {(['SMS', 'Email'] as const).map((channel) => (
                <label
                  key={channel}
                  className={cx(
                    'flex cursor-pointer items-center gap-3 rounded-xl border px-4 py-3 text-[15px] font-medium transition',
