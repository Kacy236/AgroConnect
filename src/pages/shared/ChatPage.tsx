  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col lg:h-[100dvh]">
      ...
      {showThreadOnMobile && active ? (
        <header className="flex items-center gap-3 border-b border-ink-line bg-white px-4 py-3 lg:hidden">
          <button
            type="button"
          ...
          <div className="min-w-0">
            ...
            <p
              className={cx(
                ...
                active.online ? 'text-brand-600' : 'text-ink-mute',
              )}
            >
        ...
        <header
          className={cx(
            'flex items-center gap-3 px-4 py-3 lg:px-8 lg:pt-6',
        ...
        >
          <button
            ...
            aria-label="Go back"
            className="-ml-2 flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-ink transition hover:bg-brand-50 lg:hidden"
          >
            <ChevronLeft className="h-6 w-6" />
          </button>
          <h1
            className={cx(
              'flex-1 text-xl font-bold text-ink lg:text-left',
              role === 'buyer' ? 'text-center lg:text-3xl lg:text-brand-600' : 'text-center lg:text-[22px]',
            )}
          >
            Chats
          </h1>
      ...
      <div className="flex min-h-0 flex-1 lg:gap-4 lg:px-8 lg:pb-6">
        ...
        <section
          className={cx(
            'flex min-h-0 w-full flex-col lg:w-[340px] lg:shrink-0 lg:rounded-xl lg:border lg:border-ink-line',
            showThreadOnMobile && 'hidden lg:flex',
          )}
        >
        ...
        <section
          className={cx(
            'flex min-h-0 flex-1 flex-col overflow-hidden lg:rounded-xl lg:border lg:border-ink-line',
            !showThreadOnMobile && 'hidden lg:flex',
          )}
        >
          {active ? (
              ...
              <header className="hidden items-center gap-3 border-b border-ink-line px-4 py-3 lg:flex">
                ...
                <div className="min-w-0">
                  ...
                  <p
                    className={cx(
                      'text-[15px] font-medium',
                      active.online ? 'text-brand-600' : 'text-brand-600',
                    )}
                  >
                    {active.online ? 'Online' : active.lastSeen}
                  </p>
