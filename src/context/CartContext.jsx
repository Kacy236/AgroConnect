...
export function CartProvider({ children }) {
  ...
  const increment = useCallback(
    (productId) => setLines((list) => list.map((l) => (l.productId === productId ? { ...l, qty: l.qty + 1 } : l))),
    [],
  )
  ...
  const decrement = useCallback(
    (productId) =>
      setLines((list) =>
        list.map((l) => (l.productId === productId ? { ...l, qty: l.qty - 1 } : l)).filter((l) => l.qty > 0),
      ),
    [],
  )

  const remove = useCallback(
    (productId) => setLines((list) => list.filter((l) => l.productId !== productId)),
    [],
  )
