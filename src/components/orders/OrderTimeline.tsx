...
const STEPS: Array<{ status: OrderStatus; label: string; note: string }> = [
...
export function OrderTimeline({ status }: { status: OrderStatus }) {
  if (status === 'cancelled') {
    return (
      <div className="rounded-xl bg-status-cancelledBg px-4 py-3 text-sm font-semibold text-status-cancelled">
            ...
            <div className="flex flex-col items-center">
              <span
                className={cx(
                  ...
                  done ? 'border-brand-600 bg-brand-600 text-white' : 'border-ink-line bg-white',
                )}
              >
              ...
              {!isLast ? (
                <span
                  className={cx('mt-1 w-[2px] flex-1', i < current ? 'bg-brand-600' : 'bg-ink-line')}
                />
              ) : null}
            </div>
            <div className="pb-1">
