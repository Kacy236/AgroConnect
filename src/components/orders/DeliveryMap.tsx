 ...
 */
export function DeliveryMap({ status }: { status: OrderStatus }) {
  ...
  return (
    <div className="relative w-full overflow-hidden bg-[#E8EDDF]">
      <img
        src="/img/delivery-map.jpg"
        alt="Delivery route from the farm to the delivery address"
        className="h-full w-full object-cover"
      />

      <span
        className="absolute z-10 -translate-x-1/2 -translate-y-1/2 transition-all duration-[2600ms] ease-in-out"
      ...
      <span
        className={cx(
          'absolute left-3 top-3 z-10 flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-sm font-bold text-white shadow-lift',
          status === 'delivered' ? 'bg-brand-700' : 'bg-brand-600',
        )}
      >
