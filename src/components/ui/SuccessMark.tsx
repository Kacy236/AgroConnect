...
const CONFETTI: Array<{ x: number; y: number; s: number; c: string; r: number; round?: boolean }> = [
...
export function SuccessMark({ className }: { className?: string }) {
  return (
    <div className={cx('relative mx-auto aspect-[3/2] w-full max-w-[320px]', className)} aria-hidden="true">
      {CONFETTI.map((c, i) => (
        <span
          key={i}
...
export function SuccessScreen({ title, description, children, actions }: SuccessScreenProps) {
  return (
    <div className="mx-auto flex min-h-[100dvh] w-full max-w-md flex-col items-center justify-center px-6 py-10 text-center">
      <SuccessMark />
      <h1 className="mt-4 text-[34px] font-extrabold leading-tight text-ink">{title}</h1>
      {description ? (
        <div className="mt-4 max-w-[19rem] text-[15px] leading-relaxed text-ink-soft">
          {description}
        </div>
      ) : null}
