...
export function Logo({
  ...
  return (
    <img
      ...
      alt="AgroConnect"
      className={cx('h-auto w-[150px] select-none', className)}
      draggable={false}
    />
  )
}
  ...
  return (
    <div className={cx('relative w-full', wrapClassName)}>
      ...
      <input
        ...
        {...rest}
        className={cx(
          'w-full rounded-full border border-ink-line bg-white py-3 pl-11 pr-4 text-[15px] text-ink transition placeholder:text-ink-mute focus:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-100',
          className,
        )}
      />
    </div>
...
export function StarRating({
  ...
  return (
    <div className={cx('flex items-center gap-0.5', className)} aria-label={`${value} out of 5 stars`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          key={i}
          style={{ width: size, height: size }}
          className={cx(
            i <= Math.round(value) ? 'fill-[#FFB800] text-[#FFB800]' : 'fill-ink-line text-ink-line',
          )}
        />
      ))}
    </div>
...
export function ChoiceChip({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      className={cx(
        'rounded-full border px-5 py-2.5 text-[15px] font-semibold transition',
        active
...
export function SectionHeading({
  ...
  return (
    <div className={cx('mb-3 flex items-end justify-between gap-4', className)}>
      ...
      {action && to ? (
        <Link to={to} className="text-sm font-semibold text-brand-600 hover:underline">
          {action}
        </Link>
      ) : null}
    </div>
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-ink-line px-6 py-14 text-center">
  ...
  return (
    <button
      ...
      className={cx(
        ...
        checked ? 'bg-brand-600' : 'bg-ink-line',
      )}
    >
      <span
        className={cx(
