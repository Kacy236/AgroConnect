...
interface TextFieldProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'size'> {
  ...
  /** Pill inputs match the auth screens; boxed inputs match the in-app forms. */
  shape?: 'pill' | 'box'
}
      ...
      <div className="relative">
        ...
        {isPassword ? (
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-1.5 text-ink-mute transition hover:text-ink"
  ...
  return (
    <FieldWrap label={label} hint={hint} error={error} className={wrapClassName} htmlFor={fieldId}>
      <textarea
        id={fieldId}
        {...rest}
        className={cx('field-box min-h-[104px] resize-none leading-relaxed', className)}
      />
    </FieldWrap>
  )
}
