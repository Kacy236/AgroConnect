...
function Count({ n }: { n: number }) {
  if (n <= 0) return null
  return (
    <span className="ml-auto flex h-5 min-w-[20px] items-center justify-center rounded-full bg-brand-600 px-1.5 text-[11px] font-bold text-white">
      {n}
    </span>
  )
}
...
export function DesktopSidebar({ role }: { role: Role }) {
  ...
  return (
    <aside
      className={cx(
        'sticky top-0 hidden h-[100dvh] w-[264px] shrink-0 flex-col overflow-y-auto lg:flex',
        isBuyer ? 'bg-brand-50 p-5' : 'border-r border-ink-line bg-white p-5',
      )}
    >
          ...
          {items.map((item) => (
            <li key={item.to}>
              <NavLink
                ...
                className={({ isActive }) =>
                  cx(
                    'flex items-center gap-3 font-bold transition',
                    isBuyer
                      ? cx(
                          'rounded-lg px-5 py-4 text-[17px]',
                          isActive
                            ? 'bg-brand-700 text-white shadow-card'
                            : 'bg-white text-brand-600 hover:bg-brand-100/70',
                        )
                      : cx(
                          'rounded-full px-4 py-3 text-[15px]',
                          isActive
      ...
      <button
        ...
        className="mt-6 flex items-center gap-3 rounded-full px-4 py-3 text-[15px] font-bold text-ink transition hover:bg-brand-50 hover:text-brand-700"
      >
