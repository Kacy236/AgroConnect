  ...
  return (
    <section className="card p-4">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="rounded-full p-1.5 text-ink-soft transition hover:bg-brand-50"
      ...
      <div className="grid grid-cols-7 gap-y-2 text-center">
        {WEEKDAYS.map((d) => (
          <div key={d} className="text-sm font-semibold text-ink">
            {d}
          </div>
        ))}
        {cells.map((day, i) => {
          ...
          return (
            <div key={day} className="flex items-center justify-center py-0.5">
              <span
                className={cx(
                  'flex h-8 w-8 items-center justify-center rounded-full text-[15px] font-semibold transition',
                  isToday
